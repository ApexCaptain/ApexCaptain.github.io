...
Windows 웹은 `noVNC`로 서비스 된다.  
내 경우 ingress annotation에 oauth2 proxy를 별도로 지정 해줬는데, 이에 따라 외부에서 아무나 들어올 수 없도록 추가적인 보안 레이어를 마련해두었다.

문제는 RDP와 VNC이다. Windows 자체가 `사용자명`과 `비밀번호` 이 2가지로 인증 처리를 하기 때문에  
Public으로 이 2가지 프로토콜을 열어두는 건 꺼림칙해 아예 VPN을 통해서만 들어올 수 있도록 설정 해두었다.

같은 방식으로 `MacOS`도 k8s 위에 올려서 동작시키는 게 가능하다.  

만일 Windows나 Mac에서만 돌아가는 프로그램을 컨테이너화 해야 하는 상황이라면 이와 같은 방법을 고려해볼만 하다.