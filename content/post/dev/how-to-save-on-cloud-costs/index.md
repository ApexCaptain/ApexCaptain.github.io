...
[CloudFormation](https://docs.aws.amazon.com/ko_kr/AWSCloudFormation/latest/UserGuide/Welcome.html)을 통해 클라우드 리소스의 형태로 배포한다.

CloudFormation은 AWS내 리소스를 yml 파일을 통해 구성/관리 할 수 있는 IaC 도구이다.  
처음 회사에 입사했을 때 Lambda를 쓰면 저렴하게 API 구성이 가능하다고 한 번 만들어 보라길래  
열심히 공부했던 기억이 난다. 그 덕에 IaC에 입문하게 되었다. <sub>(지금은 Terraform 쓰고 있지만...)</sub>
...
`절약 플랜`은 `예약 인스턴스`에 비해 부담되는 제약사항이 덜하다.  
...
이런 인스턴스들의 비용을 줄이기 위해 도입한 절약 플랜인데, 아이러니하게도 되려 다른 시스템으로의  
전환을 막는 장애물이 된 것이다. 그러니 추후 계획을 충분히 고려해 도입에 신중을 기하도록 하자.
...
1. **동일 가용 영역(AZ)에 리소스 배치**
    ...
    > **⚠️ 여기서 잠깐!** 
    >
    > 단, **데이터베이스와 같이 데이터 영속성이 그 무엇보다 중요한 경우**에는 예외이다.  
    > 이런건 가급적 멀티 AZ로 구성하는 걸 추천한다. <sub>(특히나 회사 생명이 걸려있다면)</sub>
    >
